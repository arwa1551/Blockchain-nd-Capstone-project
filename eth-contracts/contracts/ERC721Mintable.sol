...
contract Ownable {
    ...
    constructor () internal{
        _owner = msg.sender;
        emit ownershipChanged("Owner has changed");
    }
    ...
    event ownershipChanged(string _string);

    function transferOwnership(address newOwner) public onlyOwner {
        // TODO add functionality to transfer control of the contract to a newOwner.
        _owner = newOwner;
        // make sure the new owner is a real address
        require(newOwner != address(0), "Address is not a real address");
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public onlyOwner{        
        ...
        require(to != _tokenOwner[tokenId], "Cannot self approve");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(_owner == msg.sender || isApprovedForAll(ownerOf(tokenId), msg.sender), "Caller cannot approve");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "Only token owner can transfer!");
...
contract ArwaERC721Token is ERC721Metadata('Arwa Token', 'ATK', 'https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/'){
